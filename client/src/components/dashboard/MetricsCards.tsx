...
export default function MetricsCards({ metrics }: MetricsCardsProps) {
  ...
  const metricCards = [
    ...
    },
    {
      ...
      icon: DollarSign,
      iconColor: (metrics.outstandingAmount || 0) < 0 ? "text-green-600" : "text-orange-600",
      change: (metrics.outstandingAmount || 0) < 0 ? "Credits due" : `${((metrics.outstandingAmount || 0) / (metrics.monthlyRevenue || 1) * 100).toFixed(1)}%`,
      changeType: (metrics.outstandingAmount || 0) < 0 ? "increase" : (metrics.outstandingAmount || 0) > 0 ? "decrease" : "neutral",
      subtitle: (metrics.outstandingAmount || 0) < 0 ? "customer overpaid" : "unpaid balance",
    },
    {
